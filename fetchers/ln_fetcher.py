"""
fetchers/ln_fetcher.py — Fetch U-BT Liga Națională data via basketapi1 (RapidAPI).
...
# ── Constants ──────────────────────────────────────────────────────────────
COMPETITION_LABEL = "Liga Națională"
LN_CFG            = TEAM_IDS["competitions"]["liga_nationala"]
TEAM_ID           = LN_CFG["team_id"]       # 59003
TOURNAMENT_ID     = LN_CFG["tournament_id"] # 1948
OUTPUT_RAW        = DATA_RAW / "ln"
...
def _season_id(season: int) -> int:
    seasons = LN_CFG.get("seasons", {})
    sid = seasons.get(str(season))
    if sid is None:
        raise ValueError(
            f"season_id necunoscut pentru season={season}. "
            f"Adaugă-l în config/team_ids.json → competitions.liga_nationala.seasons"
        )
    return sid
...
def _fetch_new_matches(season_id: int, known_ids: set[int]) -> list[dict]:
    ...
    while True:
        ...
        novel = [e for e in ln if e["id"] not in known_ids]
        new_events.extend(novel)

        log.debug(f"  page {page}: {len(events)} total, {len(ln)} LN, {len(novel)} noi")

        if ln and not novel:
            break
        if not ln and known_ids:
            break
...
def _aggregate_player_stats(df: pd.DataFrame, season: int) -> pd.DataFrame:
    # Grupăm doar după player_id — slug-ul poate diferi între meciuri (bug API)
    meta = df.groupby("player_id", dropna=False)[
        ["competition", "season", "player_name", "player_slug", "position"]
    ].first().reset_index()

    grp = df.groupby(["player_id"], dropna=False)
    agg = grp.agg(
    ...
    # PIR = pts + reb + ast + stl + blk − (fga−fgm) − (fta−ftm) − to − pf
...
def _update_last_updated(season: int) -> None:
    ...
    data["competitions"]["liga_nationala"] = now
...
def run(season: int, full: bool = False) -> None:
    log.info(f"=== Liga Națională Fetcher — season {season} {'(full)' if full else '(incremental)'} ===")
...
if __name__ == "__main__":
    default_season = max(int(k) for k in LN_CFG["seasons"])
    parser = argparse.ArgumentParser(description="Fetch U-BT Liga Națională data")
    parser.add_argument(
        "--season", type=int, default=default_season,
        help=f"Anul de final al sezonului (default: {default_season})",
    )
    parser.add_argument(
